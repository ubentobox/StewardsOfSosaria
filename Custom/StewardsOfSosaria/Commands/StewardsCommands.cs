...
namespace StewardsOfSosaria.Commands
{
    public static class StewardsCommands
    {
        public static void Initialize()
        {
            ...
            CommandSystem.Register("TownTaskStartNext", AccessLevel.Player, new CommandEventHandler(OnTownTaskStartNext));
            CommandSystem.Register("TownTaskDone", AccessLevel.Player, new CommandEventHandler(OnTownTaskDone));
            CommandSystem.Register("TownTaskFail", AccessLevel.Player, new CommandEventHandler(OnTownTaskFail));
        ...
        private static void OnTownTaskStartNext(CommandEventArgs e)
        {
            TownAggregate town = StewardsRuntime.TownService.GetLastCreatedTown();
            if (town == null)
            {
            ...
            for (i = 0; i < tasks.Count; i++)
            {
                ...
                task.Status = TownTaskStatus.InProgress;
                if (StewardsRuntime.TownService.AuditSink != null)
                {
                    StewardsRuntime.TownService.AuditSink.Append(
                        ...
                        e.Mobile.Name,
                        town.TownId,
                        "Task started: " + task.TaskId.ToString() + " type=" + task.Type.ToString());
                }

                e.Mobile.SendMessage("Started task: {0} type={1} priority={2}", task.TaskId, task.Type, task.Priority);
                return;
            }

            e.Mobile.SendMessage("No executable queued task found to start.");
        }

        private static void OnTownTaskDone(CommandEventArgs e)
        {
            TownAggregate town = StewardsRuntime.TownService.GetLastCreatedTown();
            if (town == null)
            {
            ...
            if (e.Length < 1)
            {
                e.Mobile.SendMessage("Usage: [TownTaskDone <taskGuid>]");
                return;
            }
            ...
            task.Status = TownTaskStatus.Done;
            ...
            if (StewardsRuntime.TownService.AuditSink != null)
            {
                StewardsRuntime.TownService.AuditSink.Append(
                    ...
                    e.Mobile.Name,
                    town.TownId,
                    "Task completed: " + task.TaskId.ToString() + " type=" + task.Type.ToString());
            }

            e.Mobile.SendMessage("Task marked done: {0}", task.TaskId);
        }

        private static void OnTownTaskFail(CommandEventArgs e)
        {
            TownAggregate town = StewardsRuntime.TownService.GetLastCreatedTown();
            if (town == null)
            {
            ...
            if (e.Length < 1)
            {
                e.Mobile.SendMessage("Usage: [TownTaskFail <taskGuid> [reason]]");
                return;
            }
            ...
            string reason = e.Length > 1 ? e.ArgString.Substring(e.GetString(0).Length).Trim() : "Marked failed by operator.";
            if (string.IsNullOrEmpty(reason))
            {
                reason = "Marked failed by operator.";
            }
            ...
            task.Status = TownTaskStatus.Failed;
            ...
            if (StewardsRuntime.TownService.AuditSink != null)
            {
                StewardsRuntime.TownService.AuditSink.Append(
                    ...
                    e.Mobile.Name,
                    town.TownId,
                    "Task failed: " + task.TaskId.ToString() + " type=" + task.Type.ToString() + " reason=" + reason);
            }

            e.Mobile.SendMessage("Task marked failed: {0} ({1})", task.TaskId, reason);
        }
