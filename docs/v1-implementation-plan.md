...
- Added `[TownTaskNext]` command to preview the next executable queued task using current dependency state.

- Added `[TownTaskStartNext]` and `[TownTaskDone]` commands for quick in-shard task lifecycle transitions (`Queued -> InProgress -> Done`) with audit entries.

- Added `[TownTaskFail]` command to exercise explicit failure transitions and audit failure reasons in-shard.
